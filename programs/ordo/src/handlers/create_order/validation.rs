...
use crate::{
    ...
    utils::consts::FULL_BPS, OrderType, OrdoError,
};
...
pub(super) fn validate_order_args_and_calculate_fees(
    ...
    require!(
        ...
        OrdoError::OrderSameMint
    );

    let parsed_order_type =
        OrderType::try_from(args.order_type).map_err(|_| OrdoError::OrderTypeInvalid)?;
    require!(
        parsed_order_type != OrderType::LimitTP && parsed_order_type != OrderType::LimitSL,
        OrdoError::OrderTypeInvalid
    );
...
fn validate_tp_sl_args(
    ...
    require!(global_config.tp_sl_enabled == 1, OrdoError::TPSLNotEnabled);
    require!(
        args.tp_output_amount > 0 || args.sl_output_amount > 0,
        OrdoError::OrderParametersInvalid
    );

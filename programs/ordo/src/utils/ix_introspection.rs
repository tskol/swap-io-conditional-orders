...
pub(crate) fn deserialize_checked_paired_ordo_ix<T>(
...
{
    let deserialized_ix = deserialize_ordo_ix(paired_ix, paired_ix_role.label())?;
...
pub(crate) fn deserialize_ordo_ix<T>(ix: &Instruction, label: &str) -> Result<T>
where
    T: Discriminator + AnchorDeserialize,
{
...
pub(crate) fn ordo_ix_discriminator(ix: &Instruction) -> Result<&[u8]> {
    let Some(discriminator) = ix.data.get(..8) else {

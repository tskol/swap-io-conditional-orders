...
use crate::utils::ix_test_helpers::{
    ...
    ordo_instruction_with_accounts as ordo_instruction, shared_accounts, test_args_data,
...
fn search_flash_ixs_reject_unexpected_program_ids() {
    ...
    let first_after_current_loader = FakeInstructionLoader {
        instructions: vec![
            ordo_instruction(test_args_data(1), accounts.clone()),
            ordo_instruction(vec![1; 8], accounts),
    ...
    let second_before_current_loader = FakeInstructionLoader {
        instructions: vec![
            disallowed_instruction(),
            ordo_instruction(vec![1; 8], shared_accounts()),
            ordo_instruction(test_args_data(1), shared_accounts()),
        ],
        current_index: 1,
    };

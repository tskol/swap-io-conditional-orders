...
use crate::utils::ix_test_helpers::{
    instruction_with_accounts, ordo_instruction, ordo_instruction_with_accounts, shared_accounts,
...
fn ensure_end_ix_match_internal_deserializes_matching_end_ix() {
    ...
    let loader = FakeInstructionLoader {
        instructions: vec![
            ordo_instruction_with_accounts(START_DISCRIMINATOR.to_vec(), accounts.clone()),
            instruction_with_accounts(Pubkey::new_unique(), vec![3; 8], shared_accounts()),
            ordo_instruction_with_accounts(test_args_data(42), accounts),
        ],
        current_index: 0,
    };
...
fn ensure_start_ix_match_internal_deserializes_matching_start_ix() {
    ...
    let loader = FakeInstructionLoader {
        instructions: vec![
            ordo_instruction_with_accounts(test_args_data(7), accounts.clone()),
            instruction_with_accounts(Pubkey::new_unique(), vec![3; 8], shared_accounts()),
            ordo_instruction_with_accounts(END_DISCRIMINATOR.to_vec(), accounts),
        ],
        current_index: 2,
    };
...
#[test]
fn search_end_ix_rejects_short_ordo_discriminator() {
    let loader = FakeInstructionLoader {
        instructions: vec![
            ordo_instruction(START_DISCRIMINATOR.to_vec()),
            ordo_instruction(vec![7; 4]),
        ],
        current_index: 0,
    };
...
fn search_end_ix_rejects_repeated_end_or_start_ix() {
    let repeated_end_loader = FakeInstructionLoader {
        instructions: vec![
            ordo_instruction(START_DISCRIMINATOR.to_vec()),
            ordo_instruction(END_DISCRIMINATOR.to_vec()),
            ordo_instruction(END_DISCRIMINATOR.to_vec()),
        ],
        current_index: 0,
    };
...
#[test]
fn search_start_ix_rejects_short_ordo_discriminator() {
    let loader = FakeInstructionLoader {
        instructions: vec![
            ordo_instruction(vec![7; 4]),
            ordo_instruction(END_DISCRIMINATOR.to_vec()),

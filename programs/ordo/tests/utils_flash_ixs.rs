...
fn ordo_instruction(data: Vec<u8>, accounts: Vec<AccountMeta>) -> Instruction {
    Instruction {
        program_id: ordo::ID,
        accounts,
        data,
...
fn public_flash_ix_match_wrappers_read_instruction_sysvar() {
    ...
    let mut first_sysvar = TestAccount::new(solana_program::sysvar::instructions::ID, ordo::ID)
    ...
    let mut second_sysvar = TestAccount::new(solana_program::sysvar::instructions::ID, ordo::ID)
